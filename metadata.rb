name 'apache'
maintainer 'mherout'
maintainer_email 'carrofeito@hotmailcom'
license 'all_rights'
description 'Installs/Configures apache'
long_description 'Installs/Configures apache'

# version 0.2.0 - para que devuelva IP y HOSTNAME #
#version '0.2.0'

# versión 0.2.1 para automatizar este resultado con plantilla - no cambia  #
version '0.2.1'






# If you upload to Supermarket you should set this so your cookbook
# gets a `View Issues` link
# issues_url 'https://github.com/<insert_org_here>/apache/issues' if respond_to?(:issues_url)

# If you upload to Supermarket you should set this so your cookbook
# gets a `View Source` link
# source_url 'https://github.com/<insert_org_here>/apache' if respond_to?(:source_url)
